...
class pos_order(osv.osv):
    ...
    def action_invoice(self, cr, uid, ids, context=None):
        ...
        for order in self.pool.get('pos.order').browse(cr, uid, ids, context=context):
            ...
            ##SE OBTIENE FECHA/HORA  Y SE RESTA 5H A LA HORA PARA QUE LA FACTURA SE CREE CON LA FECHA QUE CORRESPONDE
            timezoneec= self.pool['res.users'].browse(cr, uid, uid, context=context).tz 
            print"TIMEZONE USUARIO=%s"%timezoneec  
            d = datetime.datetime.now()
            fecha = d - datetime.timedelta(hours = 5)
            print"TIMEZONE FECHA=%s"%fecha
                
            acc = order.partner_id.property_account_receivable.id
            inv = {
                'date_invoice': fecha, ##order.date_order,
                'name': order.name,
                'origin': order.name,
                'account_id': acc,
            ...
            ##SE CAMBIA DE ESTADO A ABIERTO PARA NO TENER ERRORES CON LA CONCILICAION DE EFECTIVO
            ##DEL MODULO POS ORDER DIRECRT RECONCILE
            inv_ref.signal_workflow(cr, uid, [inv_id],'invoice_open')
            ##inv_ref.signal_workflow(cr, uid, [inv_id], 'validate')
        
        ##SE COMENTA PARA QUE LOS CAJEROS NO PUENDAN VER LA PANTALLA DE FACTURAS
